...
setwd("/media/pprompsy/LaCie/InstitutCurie/Documents/Data/ChromSCape_analysis/datasets/scChIPseq_H3K27me3_HBCx_95_22_mouse/")
...
out = import_scExp(file_names = list.files("/media/pprompsy/LaCie/InstitutCurie/ChromSCape_inputs/scChIP_mouse_PDX/",pattern = ".tsv",full.names = T))
...
test_that("QC Filtering ", {
    ...
    expect_equal(dim(master$mat), dim(SingleCellExperiment::counts(scExp)) )
    expect_equal(rownames(master$mat), rownames(SingleCellExperiment::counts(scExp)) )
    expect_equal(table(gsub("_c.*","",colnames(master$mat))),
                 table(gsub("_BC.*","",colnames(SingleCellExperiment::counts(scExp)))))
})
...
test_that("Step 7 : dimensionality reduction", {
    ...
    expect_equal(cor(master$pca[,17], pca[,17]),1) # Is equal approximately
})
...
set.seed(3.14)
scExp_cf = consensus_clustering_scExp(scExp_cf,prefix = "",reps = 1000)
set.seed(47)
