...
if(any(c("coverage") %in% run)){
    if(length(coverages)>0){
        features = SummarizedExperiment::rowData(scExp_cf)
        message(head(features))
        message(head(genes_to_plot))
        message(any(genes_to_plot %in% features$Gene))
        if(any(genes_to_plot %in% features$Gene)){
