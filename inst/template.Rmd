...
```{r}
if(any(c("filter") %in% run)) plot_most_contributing_features(scExp)
```

### Most contributing chromosomes

```{r}
if(any(c("filter") %in% run)) plot_pie_most_contributing_chr(scExp)
```
...
if(any(c("DA") %in% run)){
  if("diff" %in% names(scExp_cf@metadata)){
    flexdashboard::valueBox(value = sum(scExp_cf@metadata$diff$summary["differential",]), caption = "Number of differential regons", icon ="fa-cube")
  }
}
```
