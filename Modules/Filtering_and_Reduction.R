...
moduleFiltering_and_Reduction <- function(input, output, session, raw_dataset_name, min_cov_cell, percentMin, quant_removal,
  ...
  withProgress(message='Processing data set...', value = 0, {
    ...
    write.table(feature[, 2:4], file=("/var/lib/shiny-server/feature.bed"), sep="\t", row.names=F, col.names=F, quote=F)
    system("bedtools sort -i /var/lib/shiny-server/feature.bed > /var/lib/shiny-server/featuresort.bed")
    system(paste0("bedtools closest -a /var/lib/shiny-server/featuresort.bed -b ", file.path("annotation", annotation_id(), "Gencode_TSS_pc_lincRNA_antisense.bed"), " -wa -wb -d> /var/lib/shiny-server/out.bed"))
    annotFeat <- read.table("/var/lib/shiny-server/out.bed", header=F)
    unlink(c("/var/lib/shiny-server/feature.bed", "/var/lib/shiny-server/featuresort.bed", "/var/lib/shiny-server/out.bed"))
