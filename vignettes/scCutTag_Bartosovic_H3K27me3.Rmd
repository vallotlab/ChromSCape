...
vignette: >
    %\VignetteIndexEntry{Example_scCutTag}
...
ChromScape_path = "/media/pprompsy/Depic_bioinfo_1/InstitutCurie/Documents/Data/ChromSCape_Data/ChromSCape_3.13_3/ChromSCape_analyses/"
...
plot_inter_correlation_scExp(
    scExp_cf,
    by = "cell_cluster",
    reference_group = "VLMC"
)

```

## Differential Analysis and TF Enrichment 

Knowing that H3K27me3 mark might repress active enhancer in vincinity to genes,
we will try to find the TF that are enriched in regions depleted of H3K27me3 in
each of the cell cluster using [ChEA3](https://maayanlab.cloud/chea3/). Since
enhancers can be at a long distance from genes TSS, we will take in account all 
regions closer to 100kbp from any genes TSS for the differential analysis.

```{r, eval=TRUE, echo=TRUE, fig.width=15}
scExp_cf = differential_analysis_scExp(scExp_cf,
                                       de_type = "one_vs_rest_fast",
                                       prioritize_genes = TRUE,
                                       max_distanceToTSS = 100000
                                       )
...
We observe that mOL show increased levels of H3K27me3 overall compared to other
groups. Let's find enriched TF in the differentialy depleted regions of each cell 
cluster.


```{r, eval=TRUE, echo=TRUE, fig.width=15}
scExp_cf = enrich_TF_ChEA3_scExp(scExp_cf,
                                 ref = "mm10",
                                 qval.th = 0.01,
                                 logFC.th = 1,
                                 min.percent = 0.01
...
Top TF enriched in genes depleted in Astrocyte vs all clusters

```{r, eval=TRUE, echo=TRUE, fig.width=5}
plot_top_TF_scExp(scExp_cf,
                  group = "Astrocytes",
                  set = "Depleted",
                  type = "Score",
                  n_top = 10
...
Percentage of significantly depleted genes targeted by the TFs in Astrocyte vs all clusters
...
It seems that SOX2/SOX1 regulons are strongly de-repressed because devoid of H3K27me3
specifically in Astrocytes vs all other clusters.
