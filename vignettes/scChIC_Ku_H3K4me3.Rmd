---
title: "scChIC-seq - H3K4me3 of human white blood cells"
output: html_document
vignette: >
    %\VignetteIndexEntry{Example_scChIC}
...
In their [paper](https://doi.org/10.1038/s41592-019-0361-7), Ku
et al. display the use of **scChIC-seq** single-cell chromatin immunocleavage sequencing
for H3K4me3 & H3K27me3 in human white blood cells. 

Here we will analyse their dataset of H3K4me3 comprising 285 cells which is very
high coverage (100,000 reads / cell). In their paper they managed to distinguish
7 different cell types, some of which were not identified.
  
  
In this article we will show the results of a basic analysis of these data using
ChromSCape application **from the raw data**.

## Data Access

The dataset is available as **Single-cell BED** files on the 
[GEO portal](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE105012).
As mutliple file formats & mutliple sampels are mixed on the GEO site,
we selected the H3K4me3 human white blood cells files and made them available
on a [dropbox repository](https://www.dropbox.com/home/ChromSCape_inputs?preview=GSE105012_scChIC_Ku_2019.zip).  
  

## Importing dataset, count on mutliple feature

First, load all the required library:
...
For this dataset, as it is H3K4me3 mark that accumulates near gene promoters, 
we can either use a gene-based annotation (promoters & genebody) or a 5kbp 
annotation. For this analysis, we will use both. We will first count the raw data
on 5kbp bins, then on genes.

```{r, eval=TRUE, echo=TRUE}
# each directory contain single-cell BED files of a single sample
input_directory = "/media/pprompsy/Depic_bioinfo_1/InstitutCurie/ChromSCape_inputs/GSE105012_scChIC_Ku_2019/"
files_dir_list = list.dirs(input_directory, recursive = F) 

out <- raw_counts_to_sparse_matrix(
  files_dir_list = files_dir_list,
  file_type = "scBED",
  bin_width = 5000
  )
...
the datasets is available at [GEO](https://www-ncbi-nlm-nih-gov.insb.bib.cnrs.fr/geo/query/acc.cgi?acc=GSE105012). 
...
ChromScape_path = "/media/pprompsy/Depic_bioinfo_1/InstitutCurie/Documents/Data/ChromSCape_Data/ChromSCape_3.13/ChromSCape_analyses/"
