...
#'
#' Identify a fixed number of common string (samples) in a set of varying strings (cells).
#' E.g. in the set "Sample1_cell1","Sample1_cell2","Sample2_cell1","Sample2_cell2" and with
    ...
    if (nb_samples > 1) {
        ...
        for (i in seq_len(nb_samples)) {
            for (j in seq_len(nb_samples)) {
                ...
                lcs = qualV::LCS(x, y)
                ls = split(lcs[6]$va, cumsum(c(TRUE, diff(
                    lcs[6]$va
                ) != 1)))
        ...
        if (length(longest_common_between_samples) > 2) {
            if (length(grep(pattern = longest_common_between_samples, samples_names))
                == nb_samples)
                samples_names = gsub(longest_common_between_samples,
                                     "",
                                     samples_names)
        }
    }
...
raw_counts_to_feature_count_files <- function(files_dir,
                                              ...
                                              verbose = TRUE,
                                              ref = "hg38") {
    stopifnot(dir.exists(files_dir),
    ...
    if (file_type == "Index_Peak_Barcode") {
        peak_file_2 = list.files(path = files_dir,
                                 full.names = TRUE,
                                 pattern = ".*peaks.bed$")
        index_file = list.files(path = files_dir,
                                full.names = TRUE,
                                pattern = ".*index.txt$")
        barcode_file = list.files(path = files_dir,
                                  full.names = TRUE,
                                  pattern = ".*barcodes.txt$")
        if (length(c(peak_file_2, index_file, barcode_file)) != 3)
            stop(
                "ChromSCape::raw_counts_to_feature_count_files - For Index Count type, the folder
    ...
    type = length(which(c(
        !is.null(peak_file),!is.null(n_bins),!is.null(bin_width),!is.null(geneTSS)
    )))
    
    if (type > 1 | type == 0) {
        stop(
            "ChromSCape::raw_counts_to_feature_count_files - peak_file, n_bins,
    ...
    if (!is.null(peak_file)) {
        cat(
            'ChromSCape::raw_counts_to_feature_count_files - Reading in peaks file...\n'
        )
        features = read.table(peak_file,
                              sep = "\t",
                              header = FALSE,
                              quote = "")
        ...
        which <-
            GenomicRanges::GRanges(
                seqnames = features$V1,
                ranges = IRanges::IRanges(features$V2, features$V3)
            )
    } else if (!is.null(n_bins) | !is.null(bin_width)) {
        cat(
            'ChromSCape::raw_counts_to_feature_count_files - Counting on genomic bins...\n'
        )
    ...
    if (file_type == "BAM") {
        ...
        if (verbose)
            cat(
                "ChromSCape::raw_counts_to_feature_count_files - Count matrix created from BAM files in",
                t1[3],
                "sec.\n"
            )
    }
        ...
        if (verbose)
            cat(
                "ChromSCape::raw_counts_to_feature_count_files - Count matrix created from BED files in",
                t1[3],
                "sec.\n"
            )
    }
    else if (file_type == "Index_Peak_Barcode") {
        name_cells = read.table(
            barcode_file,
            sep = "",
            quote = "",
        ...
        if (verbose)
            cat(
                "ChromSCape::raw_counts_to_feature_count_files - Count matrix created from Index-Peak-Barcodes files in",
                t1[3],
                "sec.\n"
            )
    }
...
bams_to_matrix_indexes = function(files_dir, which) {
    ...
    system.time({
        feature_list = BiocParallel::bplapply(names(bam_files), function(bam_name) {
            ...
            tmp = Rsamtools::countBam(file = bam_files[[bam_name]], param =
                                          param)
...
#' 
#' @return A list containing a "feature index" data.frame and a 
#' region GenomicRange object both used to form the sparse matrix
#'
...
beds_to_matrix_indexes = function(files_dir, which) {
    single_cell_beds = list.files(files_dir, full.names = TRUE, pattern = ".*.bed$|.*.bed.gz$")
    ...
    system.time({
        feature_list = BiocParallel::bplapply(names(single_cell_beds), function(bed_name) {
            ...
            bed = GenomicRanges::GRanges(setNames(bed[, seq_len(3)], c("chr", "start", "end")))
            suppressWarnings({
                          ...
                          ref = "hg38")
{
    ...
    if (anyNA(peaks_start) | anyNA(peaks_end))
        stop(
            "The rows of mat should be regions in format chr_start_end or
    ...
    hits <-
        GenomicRanges::findOverlaps(bin_ranges, peaks, minoverlap = minoverlap)
    bins_names = paste0(
        bin_ranges@seqnames,
        ":",
    ...
    t = system.time({
        bin_mat = BiocParallel::bpaggregate(x = hits[, 3:dim(hits)[2]],
                                            by = list(bins = hits[, 1, drop =
                                                                      FALSE]),
                                            FUN = sum)
    })
    cat("ChromSCape::peaks_to_bins - transformed peaks into bins in ",t[3], "sec.\n")
...
#'  * annot : a data.frame of cell annotation
#'  * batches : an integer vector with the batch number for each cell
#' @export
#'
#' @importFrom GenomicRanges GRanges tileGenome width seqnames GRangesList
#' sort.GenomicRanges
                                 ...
                                 batch_id = rep(1, nsamp))
{
    ...
    if (featureType[1] == "peak")
    {
        ...
        chr_ranges <-
            GenomicRanges::sort.GenomicRanges(unlist(chr_ranges_list))[seq_len(features)]
    ...
    if (featureType[1] == "gene")
    {
        ...
        # load species chromsizes
        features_names <-
            as.character(sample(chr$gene, features, replace = FALSE))
    }
...
import_scExp <- function(file_names,
    ...
    if (length(grep("(.tsv$)|(.txt$)|(.csv$)", file_names)) < length(file_names))
        stop(paste0(
            "ChromSCape::import_scExp - Matrix files must be in .txt or .tsv format."
        ))
    ...
    for (i in seq_along(file_names)) {
        ...
        if (is.null(datamatrix)) {
            ...
            common_regions <-
                intersect(rownames(datamatrix),
                          rownames(datamatrix_single))
            
            if (length(common_regions) > 0) {
                datamatrix <-
                    Matrix::cbind2(datamatrix[common_regions, ], datamatrix_single[common_regions, ])
            } else {
                stop(
                    paste0(
                        "ChromSCape::import_scExp - ",
                        file_names[i],
                        " contains no common regions with ",
                        file_names[i - 1]
                    )
                )
            }
            if (length(common_regions) < nrow(datamatrix)) {
                warning(
                    paste0(
                        ...
                        file_names[i],
                        " contains less than ",
                        ceiling(100 * length(common_regions) / (nrow(
                            datamatrix
                        )))
                        ,
                        " common regions with ",
                         ...
                         verbose = TRUE)
{
    stopifnot(
        is.data.frame(annot),
        remove_zero_cells %in% c(TRUE, FALSE),
    ...
    scExp <- SingleCellExperiment::SingleCellExperiment(assays = list(counts = datamatrix),
                                                        colData = annot)
                          ...
                          verbose = TRUE)
{
    ...
    sel1000 <-
        (cellCounts > 1000 & cellCounts <= thresh[quant_removal +
                                                      1])
    sel <-
        (cellCounts > min_cov_cell & cellCounts <= thresh[quant_removal +
                                                              1])
    if (verbose) {
        cat(
    ...
    gc()
    bina_counts = Matrix::sparseMatrix(
        i = 1,
        j = 1,
        x = 1,
    ...
    system.time({
        annotFeat <- annotFeat %>% dplyr::group_by(ID, chr, start, end) %>%
            dplyr::summarise(
                Gene = paste(Gene,
                             collapse = ", "),
                distanceToTSS = max(distanceToTSS)
            ) %>% as.data.frame()
    })
...
reduce_dims_scExp <-
             ...
             verbose = TRUE)
    {
        ...
        if ("UMAP" %in% dimension_reductions)
        {
            ...
            umap <-
                umap::umap(pca, config = config, method = "naive")
            umap <- as.data.frame(umap$layout)
            colnames(umap) <- c("Component_1", "Component_2")
        }
        
        # save PCA & TRUE-SNE in scExp object
