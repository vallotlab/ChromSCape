    ...
    verbose = FALSE)
{
    stopifnot(
        is(scExp, "SingleCellExperiment"),
        is.numeric(random_iter),
        is.numeric(corr_threshold),
    ...
    if (is.null(SingleCellExperiment::reducedDim(scExp, "PCA")))
        stop(
            "ChromSCape::correlation_and_hierarchical_clust_scExp - No PCA, 
    ...
    selection_cor_filtered <-
        (apply(corChIP, 1, function(x)
    ...
    tab = as.data.frame(SingleCellExperiment::reducedDim(scExp,
                                                         "Cor"))[, selection_cor_filtered]
...
num_cell_before_cor_filt_scExp <- function(scExp)
{
    ...
    table = table %>% dplyr::bind_rows(., dplyr::tibble(Sample = "", `#Cells` = sum(table[,-1])))
    
    table %>% dplyr::mutate(Sample = cell_spec(
        Sample,
        color = "white",
        bold = TRUE,
        background = colors
...
#'
consensus_clustering_scExp <- function(
    scExp,
    prefix = NULL,
    maxK = 10,
    reps = 100,
    ...
    plot_icl = "png")
{
    ...
    icl <-
        ConsensusClusterPlus::calcICL(consclust, plot = plot_icl, title = prefix)
    ...
    hc_linkage = "ward.D")
{
    ...
    cell_clusters_list <-
        lapply(unique(cell_clusters), function(z)
            names(which(cell_clusters ==
                            z)))
    
    scExp = colors_scExp(
        scExp = scExp,
        annotCol = "cell_cluster",
...
num_cell_in_cluster_scExp <- function(scExp)
{
    ...
    table_raw <-
        as.data.frame.matrix(table(as.data.frame(
    ...
    # clusters ?
    chi <-
        suppressWarnings(stats::chisq.test(x = as.matrix(table_raw), correct = FALSE))
    ...
    for (i in seq_len((dim(as.matrix(table_raw))[1])))
    {
        ...
        chi <-
            suppressWarnings(stats::chisq.test(x = contingency_tab, correct = FALSE))
    ...
    tab <-
        cbind(tab, `#Cells` = c(Matrix::rowSums(table_raw), sum(Matrix::rowSums(table_raw))))
    tab <- cbind(tab, `p-value` = chi_pvalues)
    tab <-
        as.data.frame(cbind(Cluster = c(rownames(tab)[seq_along(rownames(tab)) -
                                                          1], ""), tab))
    ...
    samples = kableExtra::cell_spec(
        colnames(tab)[2:(length(colors_sample_id) + 1)],
        color = "white",
        bold = TRUE,
        background = colors_sample_id
    )
    ...
    tab[2:(length(colors_chromatin_group) + 1), "Cluster"] = kableExtra::cell_spec(tab[2:(length(colors_chromatin_group) +
                                                                                   ...
                                                                                   color = "white",
                                                                                   bold = TRUE,
                                                                                   background = colors_chromatin_group)
